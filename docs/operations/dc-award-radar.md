...
| `category` | `category` | Optional vertical: `hospital` / `dot_civil` / `industrial_warehouse` / `k12_higher_ed` / `data_center` / `multifamily` / `hospitality_mixed_use` / `federal_municipal` / `energy_renewables`. Alias: `vertical`. Labels with `/` or spaces (e.g. `K-12 / higher ed`, `Hospitality / Mixed Use`, `Energy / Renewables`) normalize on parse; `Multi-Family` → `multifamily`; `renewables` → `energy_renewables`. Empty / omitted = leave existing (legacy DC rows stay blank). Do not wipe on Phase 2 re-import. |
...
- A compact **lead-count strip** sits above the list (signals, campuses in grouped mode, unique contacts, phone / email / LinkedIn / cell, high-confidence signals). Counts come from Convex `dcAwardSignals.leadStats`: one indexed `.take(4000 + 1)` over the **active filters** (market, confidence, category, contact-channel chips). Unique-contact identity is the same as the list (company+name, then email/phone/LinkedIn) and is computed on the full scanned set — not page 1 only. If the scan cap is hit, chips stay visible and the footnote says **partial**. Loaded-page summarize is fallback while the query loads or if Load all already scanned more than the cap. No `.collect()`, no scheduler.
...
- **Contact filters** (Has phone / Has email / Has LinkedIn / Has cell / Missing phone / Missing email) apply to unique contacts in both Grouped and Flat views. The lead-count strip follows those filters on the **full scanned set** (`leadStats`). Table / CSV / GHL still use loaded pages. Collapse all / Expand all is unchanged.
- **Download contacts CSV** is client-side from the already-loaded, filtered unique contacts (company + contactName dedupe).
- **Send filtered contacts to GHL** confirms **sending all N** GHL-eligible (email or phone) + “no email/SMS”. There is **no 100-contact send cap**; HighLevel writes may chunk internally for rate limits. `ok: false` / error toast when created+updated is 0. Unset credentials → tag-only CSV for the **same full eligible set**. **Download contacts CSV** remains the full filtered unique set.
