...
| **DC award radar** | Convex `dcAwardSignals` (platform catalog) | `/operations/dc-award-radar` | `AppChrome` `<main>` | One-shot Phase 2 / nationwide / contact upsert (operator-secret) | Public DLC permit/news signals, any US market. Contact enrichment is Hermes/ops → CSV. **No scrape, cron, GHL, or outbound.** See `docs/operations/dc-award-radar.md`. |
