...
| **Web Push (PWA)** | Lean phone push via browser push networks; subscriptions in `pushSubscriptions`; send from `webPushActions.trySendWebPush` on existing `dispatchUserNotification` events (**`document_activity`** / client vault uploads only). | `userNotifications`, Settings toggle, `public/sw.js`. | **Implemented** lean path — see `docs/pwa-web-push.md`. | More categories via allowlist only; quiet hours optional later. |
