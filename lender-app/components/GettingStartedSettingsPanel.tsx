...
export function GettingStartedSettingsPanel() {
  ...
  const demoSummary =
    demoStatus === undefined
      ? "Checking…"
      : demoStatus.loaded
        ? `Loaded (${demoStatus.counts.pipeline} files)`
        : "Not loaded";
  ...
  return (
    <div className="max-w-xl space-y-3">
      ...
      {state?.skipped ? (
        <p
          className="rounded-dlc-md border border-amber-500/35 bg-amber-500/10 px-3 py-2 text-xs text-amber-950 dark:text-amber-50"
          role="status"
        >
      ...
      {state && !state.skipped && state.gettingStartedDismissed ? (
        <p
          className="rounded-dlc-md border border-border/80 bg-muted/40 px-3 py-2 text-xs text-muted-foreground"
          role="status"
        >
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button
          ...
          data-testid="settings-demo-workspace-open"
        >
          Demo workspace…
        </Button>
      </div>
      <p className="text-xs text-muted-foreground">
        Demo sample data:{" "}
        <span className="font-medium text-foreground">{demoSummary}</span>
      </p>
      ...
      {demoModalOpen ? (
        <ActionSuiteModal
          ...
          testId="settings-demo-workspace-modal"
        >
          <div className="max-h-[min(70dvh,28rem)] space-y-3 overflow-y-auto overscroll-contain">
            <p className="text-xs text-muted-foreground">
              ...
              <span className="font-medium text-foreground">[Demo]</span> and
              tagged so you can delete the whole bundle anytime (nothing touches
              your real data).
            </p>
            {!activeOrganizationId ? (
              <p className="text-xs text-muted-foreground">
              ...
              <div className="space-y-2">
                {demoStatus.loaded ? (
                  <p className="text-xs text-muted-foreground" role="status">
                    ...
                    {demoStatus.counts.lenders} lenders,{" "}
                    {demoStatus.counts.tasks} tasks.
                  </p>
                ) : null}
                {demoStatus.loadBlockedReason ? (
                  <p
                    className="rounded-dlc-md border border-amber-500/30 bg-amber-500/5 px-2 py-1.5 text-[11px] text-amber-950 dark:text-amber-50"
                    role="status"
                  >
                    {demoStatus.loadBlockedReason}
                  </p>
