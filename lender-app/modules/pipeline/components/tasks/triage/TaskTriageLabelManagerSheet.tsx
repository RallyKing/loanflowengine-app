...
export function TaskTriageLabelManagerSheet({
  ...
  return (
    <OperationalOverlayShell
      ...
      panelClassName={cn(
        "flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden p-0",
        narrow && "max-h-[min(85dvh,720px)] rounded-b-none sm:rounded-b-dlc-lg",
      )}
    >
      <div className="flex max-h-[85vh] min-h-0 flex-col overflow-hidden">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6 sm:px-5 sm:py-3">
          {showForm ? (
            ...
            <div className="space-y-3">
              ...
              <ul className="space-y-2" data-testid="triage-label-manager-list">
                {labels.map((row, index) => {
                  const hex = resolveTriageLabelHex(row, colorPresets);
                  return (
                    <li
                      key={row._id}
                      className="flex items-center gap-2 rounded-dlc-md border border-border/60 bg-background px-2 py-2"
                    >
                      <div className="flex shrink-0 flex-col gap-0.5">
                        <button
                          type="button"
                          className="inline-flex h-7 w-7 items-center justify-center rounded-dlc-sm text-muted-foreground hover:bg-muted/50 disabled:opacity-40"
