...
function RegistryFilterDropdown({
  label,
  summary,
  children,
  testId,
...
export function RegistryCommandBar({
  searchInput,
  onSearchInputChange,
  typeFilters,
  onTypeFiltersChange,
  ...
  return (
    <div
      className={cn(
        "sticky top-0 z-40 shrink-0 border-b border-slate-200/90 bg-white/95 px-4 py-3 backdrop-blur-sm",
        "dark:border-slate-700/80 dark:bg-dlc-surface-high/95",
      )}
      data-testid="registry-command-bar"
    >
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-3">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          {canMutate ? (
            <DropdownMenu
              align="start"
              aria-label="Add new registry record"
              trigger={
                <Button
                  type="button"
                  variant="primary"
          ...
          <div className="ml-auto flex min-w-0 flex-1 flex-wrap items-center justify-end gap-2 sm:flex-nowrap">
            <div className="relative min-w-[10rem] flex-1 sm:max-w-[16rem]">
              ...
              <Input
                ...
                aria-label="Search registry by name, email, or phone"
              />
            </div>
                ...
                return (
                  <FilterToggleRow
                    ...
                    onToggle={() => {
                      ...
                      const next = toggleArrayItem(typeFilters, id);
...
/** Debounced search hook for registry list queries. */
export function useRegistrySearchDebounce(
  searchInput: string,
