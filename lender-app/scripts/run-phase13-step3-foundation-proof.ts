...
async function main() {
  ...
  const proof = convexRun(
    "operator/pipelineHierarchyFoundationStep13_3:runHierarchyFoundationProof",
    { adminSecret },
  );

  const report = {
    generatedAt: Date.now(),
    phase: "13.3-step2-foundation",
    proof,
    validation: {
      dataMigration: "none",
      schema: "additive_clients_projects_pipeline_fks",
      uiChanges: "none",
    },
  };
  writeFileSync(
    join(reportsDir, "phase13-step3-foundation.json"),
