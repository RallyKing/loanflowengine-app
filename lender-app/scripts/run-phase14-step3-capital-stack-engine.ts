#!/usr/bin/env npx tsx
/**
 * Phase 14 Step 3 — capital stack engine production proof.
 */
...
async function main() {
  ...
  const proof = convexRun(
    "operator/pipelineCapitalStackStep14_3:runCapitalStackProof",
    { adminSecret },
  );

  const report = {
    generatedAt: new Date().toISOString(),
    phase: "14-step3-capital-stack-engine",
    pass: Boolean((proof as { pass?: boolean }).pass),
    proof,
  };

  const outPath = join(reportsDir, "phase14-step3-capital-stack-engine.json");
