#!/usr/bin/env npx tsx
/**
 * Phase 14 Step 2 — multi-client UI production proof.
 */
...
async function main() {
  ...
  const proof = convexRun(
    "operator/pipelineMultiClientUiStep14_2:runMultiClientUiProof",
    { adminSecret },
  );

  const report = {
    generatedAt: new Date().toISOString(),
    phase: "14-step2-multi-client-ui",
    pass: Boolean((proof as { pass?: boolean }).pass),
    proof,
  };

  const outPath = join(reportsDir, "phase14-step2-multi-client-ui.json");
