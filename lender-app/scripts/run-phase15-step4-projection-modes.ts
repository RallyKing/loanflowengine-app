#!/usr/bin/env npx tsx
/**
 * Phase 15 Step 4 — projection modes production proof report.
 */
...
async function main() {
  ...
  const proof = convexRun(
    "operator/indexedGraphProjectionProofStep15_4:runProjectionProofStep15_4",
  ...
  const report = {
    ...
    phase: "15-step4-projection-modes",
    organizationId: JOSHUA_ORG,
    proof,
    pass: proof.pass === true,
  };

  const outPath = join(reportsDir, "phase15-step4-projection-modes.json");
  writeFileSync(outPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
  console.log(`Wrote ${outPath}`);
  console.log(JSON.stringify({ pass: report.pass, proof }, null, 2));
