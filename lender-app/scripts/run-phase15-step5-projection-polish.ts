#!/usr/bin/env npx tsx
/**
 * Phase 15 Step 5 — projection polish production proof report.
 */
...
async function main() {
  ...
  const proof = convexRun(
    "operator/indexedGraphProjectionPolishProofStep15_5:runProjectionPolishProofStep15_5",
  ...
  const report = {
    ...
    phase: "15-step5-projection-polish",
    organizationId: JOSHUA_ORG,
    proof,
    pass: proof.pass === true,
    manualVerification: [
      "Remove a secondary client from a file in workspace header / LinkedClientsEditor",
      "Confirm fileClients edge deleted; pipeline row and clients record remain",
      "Add a different secondary client to the same file",
      "Search Smith in Client Focus — only matching clients at top level",
      "Project Focus top level shows projects only; expand shows clients + files",
      "File Focus flat list with clickable client/project badges opens hub filters",
    ],
  };

  const outPath = join(reportsDir, "phase15-step5-projection-polish.json");
