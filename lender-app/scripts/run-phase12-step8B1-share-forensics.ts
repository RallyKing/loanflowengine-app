#!/usr/bin/env npx tsx
/**
 * Phase 12.2 Step 8B.1 — share-path forensics (production).
 */
...
async function main() {
  ...
  const emailAudit = convexRun("operator/shareForensicsStep8B1:auditCanonicalEmailLookup", {
    adminSecret,
  });
  const liveProof = convexRun("operator/shareForensicsStep8B1:runLiveSharePathProof", {
    adminSecret,
  });
  ...
  writeFileSync(
    join(reportsDir, "phase12-step8B1-share-forensics.json"),
