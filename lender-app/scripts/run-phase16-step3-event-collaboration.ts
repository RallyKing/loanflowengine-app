...
async function main() {
  ...
  const proof = convexRun(
    "operator/eventCollaborationStep16_3:runEventCollaborationProof",
    { adminSecret },
    true,
  );
  ...
  const report = {
    ...
    phase: 16,
    step: 3,
    title: "Event shell CRUD + collaborative sharing",
    proof,
    pass: Boolean(proof.pass),
  };

  writeFileSync(
    join(reportsDir, "phase16-step3-event-collaboration.json"),
    JSON.stringify(report, null, 2),
  );

  const steps = (proof.steps as Array<{ name: string; pass: boolean }>) ?? [];
  writeFileSync(
    join(docsDir, "phase16-step3-event-collaboration.md"),
    `# Phase 16 Step 3 — Event collaboration
...
No calendar, print engine, or Step 4. Await operator review.
`,
  );
